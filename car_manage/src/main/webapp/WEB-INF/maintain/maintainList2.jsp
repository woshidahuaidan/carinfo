		...
		<div class="container">
			<div class="row">
				<div class="col-md-8 col-md-offset-2">
					<a href="useCar/addList.do"><h2 class="text-center">维修信息</h2></a>
				</div>
			</div>
		</div>
	</div>
...
function pageQuery(pageIndex){
	 ...
	 var pageno = pageIndex+1;
	  ...
	  $.ajax({
		  type : "POST",
	  	  url   : "maintain/pageQueryMaintain.do",
	  	  data  : {
	  		  pageno : pageno,
	  		  pagesize : pagesize
	  	  },
	  	  success : function(result){
	  		  if(result.success){
	  			  var maintainPage = result.data;
	  			  var maintains = maintainPage.datas;
	  			  var content = "";
	  			  $.each(maintains,function(index,maintain){
	  				  var mtime = new Date(maintain.mtime).toLocaleDateString();
	  				  content += '<tr>';
	                  content += '<td>'+(index+1)+'</td>';
					  content += '<td>'+maintain.mid +'</td>';
	                  content += '<td>'+ mtime +'</td>';
	                  content += '<td>'+ maintain.mmoney   +'</td>';
	                  content += '<td>'+ maintain.maddr   +'</td>';
	                  content += '<td>'+ maintain.mreason   +'</td>';
	              	  content += '<td>'+ maintain.cno   +'</td>';
	                  content += '<td>'+ maintain.dname   +'</td>';
	                  content += '</tr>';
	  			  });
	  			  $("#tbody").html(content);
	  			  
	  			$(".pagination").pagination(maintainPage.totalsize, {
	...
	function addList(){
		window.location.href = "maintain/addList.do";
	}
</script>
</body>
</html>