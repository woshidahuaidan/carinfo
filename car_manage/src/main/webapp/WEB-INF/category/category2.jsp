 	...
 	<div class="easyui-layout" data-options="fit:true">
		...
		<div id="m" class="easyui-window" title="Window Layout" closed="true"  style="width:500px;height:300px;padding:5px;">
			<div style="width:400px;padding:20px;margin:20px">
				<form id="mm" action="category/addLevels.do" method="post">
				<input type="hidden" name="id" id="id" >
					<table cellpadding="5">
						...
						<tr>
							<td class="tt">级别：</td>
							<td><input class="easyui-combobox" id="levelid"name="level" style="width:200px;" 
											data-options="
											valueField:'id',
											multiple:true,
											textField:'level',
											url: 'category/levelList.do',
											columns:[[
													{field:'level'}
											]]">
							</td>
						...
						<tr>
							<td class="tt"></td>
							<td><input id="addLevelBtn" type="button" value="提交" onclick="addLevels()"/> </td>
						</tr>
					</table>
				</form>
			</div>
...
<script type="text/javascript">
		$(function(){
			$('#ta').datagrid({    
			    ...
			    columns:[[    
			        ...
			        {field:'id',title:'分类编号',width:100,align:'center'},    
			        {field:'ccname',title:'分类名称',width:100,align:'center'}, 
		...
		function addLevels(){
			var row = $('#ta').datagrid("getSelected");
			alert(row.id);
			var checkedItems = $('#levelid').combobox("getValues");
		    ...
		    console.log(json);
		    alert(3);
		    $.post("category/addLevels.do",json,function(data){
		    	if(data=="1")
		    		 $('#m').dialog('close');    // 关闭窗口
		    })
		}
		...
		function addOrUpdateCa(){
			...
			$.post('category/addOrUpdate.do',json,function(data){
				if(data=="1"){
					 $.messager.show({
						 title:'我的消息',
							msg:'添加成功',
		...
		function deleteBatch(){
			...
			$.messager.confirm('确认对话框', '确定要批量删除记录吗？', function(r){
				if(r){
					console.log(json);
					$.post('category/deleteBatch.do',json,function(data){
						if(data=="1"){
							 $.messager.show({
									title:'我的消息',
									msg:'批量删除成功',
